 ...
 * D14 (naming a text box) and D15 (the Rows tab) riding in on the screens they extend: none of the six has a
 * build file of its own, so nothing is imported here for them.
 *
 * Two of D13b's and D14's states stay pictures on their own canvases rather than clicks here, decided by Garreth
 * on 2026-09-22 when the prototype was rebuilt for them: a type with nothing written (the prototype holds one
 * sample type page and that one has its Writing saved), and a text box added from the Studio's tool strip (D6's
 * pictures-first rule). Both name their board in a note, the way a type that is not wired yet already does.
 *

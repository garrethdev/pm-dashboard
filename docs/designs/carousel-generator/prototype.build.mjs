 ...
 * (the Studio's second round), D12 (Auto mode), D13 (Writing and Go Live), D13b (the Conversation's first draft),
 * D14 (naming a text box), D15 (the Rows tab) and D17 (mentions in the Writing) riding in on the screens they
 * extend: none of the seven has a build file of its own, so nothing is imported here for them. D17 is the one
 * that needed something passed in — `typeScreen({ live: true })` — because it is behaviour rather than a state.
 *
 * Four states stay pictures on their own canvases rather than clicks here. Two were decided by Garreth on
 * 2026-09-22 when the prototype was rebuilt for D13b and D14: a type with nothing written (the prototype holds
 * one sample type page and that one has its Writing saved), and a text box added from the Studio's tool strip
 * (D6's pictures-first rule). Two more came with D17 and are limits of the click-through rather than decisions:
 * a Writing that names a box which no longer exists, because nothing here renames a text box — that happens in
 * the Studio, where naming one is itself a picture — and the Conversation answering with the same mentions back,
 * because Send on this screen has always reported itself rather than posting a message. All four name their
 * board in a note, the way a type that is not wired yet already does.
 *
...
import { overviewScreen, copyOverviewImages } from "./d16-overview.build.mjs";
...
const SCREENS = () => [
  /* D16 Overview (approved dark and light 2026-09-22): the generator's front page, and the prototype's
     first screen. It gathers Today's numbers, Running Tasks, the five most recently generated types,
     and what Trends scraped or the person saved; its rows open the type page, the Generate form, a
     running batch, Carousel types and Trends. Its menu item is the way back to it from anywhere. */
  overviewScreen(),
  ...
  /* D7 A carousel type's page (approved 2026-09-15 after five review rounds): a type's name on D1 and Edit on D2's
     direction open it. Its tab row is Overview · Writing · Rows · Go Live: D13 renamed the first and last, and
     D15 (approved 2026-09-22) added Rows between them, so the tab and its drawer arrive here with D7's screen.
     `live` turns on D17 (approved 2026-09-22): the text-box pills under the Writing become how a box's name gets
     into it — dragged in, pressed, or typed with `@` — and the editor becomes the surface that can draw a
     mention as a pill. The boards stay pictures; only the click-through takes this. */
  typeScreen({ live: true }),
...
const clashes = shellClashes(SCREENS(), { start: "overview" });
if (clashes.length) {

...
function vals({ auto, init }) {
  return `
    ...
    var seed = function (p) {
      ...
      /* D12: what Auto gave up on while the copy was being written. It arrives already dropped — never rendered,
         never approved, still there to be read and regenerated by hand. */
      var dropList = (p && p.dropped) || [];
      /* Reopened from a screen that said this batch was waiting for its sign-off (D1's card, D7's table, D9's
         Status, the bell): its decks are already rendered and it is the Approve press that is still to come. */
      var done = !!(p && p.finished);
      var list = [];
      for (var n = 1; n <= total; n++) {
        if (gone.indexOf(n) >= 0) continue;
        var extra = dropList.indexOf(n) >= 0
          ? { st: "dropped", done: SLIDES, tries: 3, check: { k: 4, why: "cut" } }
          : flagList.indexOf(n) >= 0 ? { st: "flagged", flag: REVIEW_FLAG }
            : done ? { st: "rendered", done: SLIDES } : {};
        list.push(Object.assign({}, BASE, { n: n }, extra));
      }
      return list;
    };
    ...
    /* D12: Auto read at the moment the timer fires — Pause has to stop the retries the instant it is pressed. */
    var a12 = function () {
      var st = self.state || {};
      return st.d12auto != null ? st.d12auto : ((st.params || {}).auto || "");
    };
    var tick = function () {
      var st = self.state || {};
      ...
      var cur = (st.d5decks || seed(st.params)).map(function (d) {
        if (d.st === "rendering") {
          ...
          var fin = next >= SLIDES;
          var check = fin ? (d.n === 4 && !d.mended ? { k: 4, why: "cut" } : d.n === 9 ? { k: 6, why: "contrast" } : null) : d.check;
          /* D12: the check after rendering is a flag like any other, so in Auto the deck goes round again rather
             than waiting to be noticed — three tries, then it is dropped and the batch finishes without it.
             Deck 4 comes good on its second pass; deck 9 never does. */
          if (fin && check && a12() === "on") {
            var tries = (d.tries || 0) + 1;
            if (tries >= 3) return Object.assign({}, d, { done: next, freshK: 0, st: "dropped", tries: 3, check: check });
            if (d.n === 4) return Object.assign({}, d, { done: 0, freshK: 0, st: "rewriting", tries: tries, check: null, mended: true });
            return Object.assign({}, d, { done: 0, freshK: 0, st: "rewriting", tries: tries, check: check });
          }
          return Object.assign({}, d, { done: next, freshK: next, st: fin ? "rendered" : "rendering", check: check });
        }
        /* D12: a render that fell over is a flag like the rest — in Auto the batch presses Retry itself, three
           times, and drops the deck if the third one falls over too. Nobody is asked, so nothing rings. */
        if (d.st === "failed" && a12() === "on") {
          moved = true;
          var ftries = (d.tries || 0) + 1;
          return ftries >= 3
            ? Object.assign({}, d, { st: "dropped", tries: 3, failSlide: 0, freshK: 0, check: { k: d.failSlide || 1, why: "cut" } })
            : Object.assign({}, d, { st: "rendering", done: Math.max(0, d.failSlide - 1), failSlide: 0, retried: true, tries: ftries, freshK: 0 });
        }
      ...
      if (!moved) {
        clearInterval(self.d5timer); self.d5timer = null;
        /* D12, in the prototype: the batch has finished and nobody has approved it yet, so it says so on the
           shell. Carousel types then carries "n to approve" wherever the person goes next, which is the whole
           point of the waiting states — a finished batch cannot sit unnoticed on one screen while another
           calls it done. */
        var left = cur.filter(function (d) { return d.st !== "dropped" && d.st !== "flagged"; });
        var allIn = left.length > 0 && left.every(function (d) { return d.st === "rendered" || d.st === "approved"; });
        var toAppr = left.filter(function (d) { return d.st === "rendered"; }).length;
        if (allIn && toAppr > 0) {
          self.setState({ d12waiting: {
            count: toAppr,
            text: toAppr + " to approve",
            to: "render",
            total: cur[cur.length - 1].n,
            dropped: cur.filter(function (d) { return d.st === "dropped"; }).map(function (d) { return d.n; })
          } });
        }
      }
    };
    var working = decks.some(function (d) { return d.st === "queued" || d.st === "rendering" || d.st === "elsewhere" || d.st === "rewriting"; });
    /* D12: the same as on the writing screen — in Auto a deck that failed or that the check flagged is work the
       batch can pick up itself, so Resume starts the painter again. */
    var a12Work = A12 === "on" && decks.some(function (d) { return d.st === "failed" || (d.st === "rendered" && !!d.check); });
    if (AUTO && s.screen === "render" && (working || a12Work) && !self.d5timer) self.d5timer = setInterval(tick, 480);
    ...
    return {
      ...
      rnApprove: function () {
        ...
        /* D12: signed off, so the batch stops asking for its person on every other screen. */
        self.setState({ d5decks: next, d12waiting: null });

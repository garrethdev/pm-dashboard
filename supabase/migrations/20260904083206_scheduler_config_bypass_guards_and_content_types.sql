-- STEP 3. Two changes:
--  1. bypass_guards now actually lifts the age ramp and the health throttle.
--     The delivery brake is deliberately NOT bypassed: it is a mechanical fact
--     (>=4 attempts, >=80% failing in 7d) rather than a judgement, and posting
--     harder into a dead proxy only burns content.
--  2. throttle_reason now explains the 9-15 and 16-22 ramp tiers, which were
--     silently clamped with a NULL reason. Also surfaces only_content_types so
--     the Smart Scheduler can read it from the same row as the caps.
create or replace view public.v_scheduler_account_config as
 WITH fleet AS (
         SELECT max(scheduler_buckets.quota_value) FILTER (WHERE scheduler_buckets.bucket = 'glp'::text) AS glp_week,
         ...
         SELECT a.geelark_profile, a."character", a.platform, a.account_created_on
           FROM accounts a
         ...
         SELECT scheduler_overrides.scope, scheduler_overrides.scope_key, scheduler_overrides.bucket,
            scheduler_overrides.weekly_cap, scheduler_overrides.daily_cap, scheduler_overrides.max_posts_per_day,
            scheduler_overrides.min_gap_minutes, scheduler_overrides.spacing_minutes,
            scheduler_overrides.window_start, scheduler_overrides.window_end,
            scheduler_overrides.bypass_guards, scheduler_overrides.only_content_types
           FROM scheduler_overrides
          WHERE scheduler_overrides.active IS TRUE
        ), base AS (
         SELECT x.geelark_profile, x."character", x.platform, x.account_created_on,
                CASE WHEN x.account_created_on IS NULL THEN 0
            ...
            COALESCE(oa_f.weekly_cap, oc_f.weekly_cap, 10) AS fil_week_cap,
         ...
         SELECT b.*,
                -- With the guards bypassed the account's own number stands at
                -- every age; otherwise the ramp tier applies.
                CASE WHEN b.bypass_guards THEN b.day_cap
                     WHEN b.age_days < 9 THEN 0
                     WHEN b.age_days <= 15 THEN 1
                     WHEN b.age_days <= 22 THEN 2
                     ELSE b.day_cap END AS ramp_day_cap,
                CASE WHEN b.bypass_guards THEN b.day_cap
                     WHEN b.age_days <= 15 THEN 0
                     WHEN b.age_days <= 22 THEN 1
                     ELSE b.day_cap END AS ramp_glp_cap,
                CASE WHEN b.bypass_guards THEN NULL
                     WHEN b.age_days < 9 THEN 0
                     WHEN b.age_days <= 15 THEN 1
                     WHEN b.age_days <= 22 THEN 2
                     ELSE NULL END AS ramp_only_cap
           FROM base b
        ), resolved AS (
         SELECT r.geelark_profile, r."character", r.platform, r.account_created_on, r.age_days,
            r.glp_week_cap, r.fil_week_cap, r.fil_day_cap, r.day_cap,
            r.min_gap_minutes, r.spacing_minutes, r.window_start, r.window_end,
            r.ramp_day_cap, r.ramp_glp_cap, r.bypass_guards, r.only_content_types,
            CASE WHEN r.bypass_guards THEN NULL
                 ELSE NULLIF(LEAST(COALESCE(r.ramp_only_cap, 2147483647),
                                   COALESCE(p.max_posts_per_day, 2147483647)), 2147483647)
            END AS guard_cap,
            ...
            CASE WHEN r.bypass_guards THEN r.ramp_day_cap
                 ELSE LEAST(r.ramp_day_cap, COALESCE(p.max_posts_per_day, r.ramp_day_cap)) END AS eff_day_cap,
            CASE WHEN r.bypass_guards THEN r.ramp_glp_cap
                 ELSE LEAST(r.ramp_glp_cap, COALESCE(p.max_glp_per_day, r.ramp_glp_cap)) END AS eff_glp_cap
           FROM ramped r
 ...
 SELECT geelark_profile, "character", platform, account_created_on, age_days, health,
    glp_week_cap, fil_week_cap,
    ...
    min_gap_minutes, spacing_minutes,
    ...
    deliv_attempts_7d, deliv_fails_7d,
    round(deliv_fails_7d::numeric / NULLIF(deliv_attempts_7d, 0)::numeric, 2) AS deliv_fail_rate_7d,
    -- Never bypassed: a dead proxy is a fact, not an opinion.
    COALESCE(NOT (deliv_attempts_7d >= 4 AND (deliv_fails_7d::numeric / NULLIF(deliv_attempts_7d, 0)::numeric) >= 0.80), true) AS can_deliver,
        CASE
            WHEN deliv_attempts_7d >= 4 AND (deliv_fails_7d::numeric / NULLIF(deliv_attempts_7d, 0)::numeric) >= 0.80
                THEN ((('delivery: '::text || deliv_fails_7d) || '/'::text) || deliv_attempts_7d) || ' failed in 7d'::text

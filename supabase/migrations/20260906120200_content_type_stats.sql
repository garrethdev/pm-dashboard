-- Per-content-type performance for the Content Types page.
--
-- content_type_score() already scores lanes, but it gates on
-- `r.active and r.unified_poster_active`, so a paused or retired lane vanishes
-- from it entirely. This page has to show exactly those — you pause a lane
-- because of its numbers, and you need to keep seeing the numbers afterwards to
-- decide whether to bring it back. So the registry is the driving side of the
-- join here and every row survives, scored or not.
--
-- The blend and its weights are copied from content_type_score() deliberately:
-- two different scores for the same lane in two places is worse than the
-- duplication. Retune both together.
...
ahead as (
  select up.content_type, count(*) as n
   ...
   where lower(coalesce(up.posting_status, '')) in ('ready', 'scheduled', 'hold')
     ...
     and (up.posting_date at time zone 'America/New_York')::date

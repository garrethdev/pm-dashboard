...
| 09-23 | `accounts_moved_to_device_at` | PF-03: when an account moved off Cloud onto a real phone, written in the same update as `delivery_mode` and `device_id` by the Settings move. The before/after line for PF-10. Nullable, no default, so every existing row reads NULL (never moved) — checked: 0 of 64 filled. Recorded as version `20260923064946`; MD5 of the recorded statements matches the file. Applied right after the database's 2am ET outage that day, which is why it went in hours after the code |
| 09-23 | `move_accounts_onto_devices` | PF-15: the batch move in Settings, all or nothing. One function checks every account and phone under lock (Cloud only, not retired, phone exists and is switched on — the single move's refusals), then sets `delivery_mode`, `device_id`, `moved_to_device_at`, the status-note line and writes one `delivery_mode_change` audit row per account, in one transaction. A refusal raises (P0001) naming the account in `detail`, and nothing is written. `posting_paused` untouched; no cap per phone. EXECUTE revoked from `public`, `anon` and `authenticated` by name; `proacl` read back as `postgres` and `service_role` only. **This version never worked**: it read the batch with `jsonb_to_recordset ... WITH ORDINALITY` and a column list, which Postgres refuses, and PL/pgSQL only checks that on the first run. It failed on the first practice press with nothing written |
| 09-23 | `move_accounts_onto_devices_fix` | PF-15, minutes later: the same function reading the batch with `jsonb_array_elements ... WITH ORDINALITY`. Same rules and signature; ACL read back unchanged. Proven live on practice rows (two phones, four accounts, all deleted afterwards): four moved in one call through the app with an audit row each; with one account moved to Physical first, the batch was refused naming it and none of the other three moved; a switched-off phone was refused naming the account |
| 09-23 | `move_account_to_cloud` | PF-03 follow-up (Garreth): the move back to Cloud in one transaction under a row lock. Sends the content behind the account's queued deliveries back to the pool by `release_profile_content`'s rule (per row, only rows still on that profile, never Posted/Posting), closes those deliveries as `skipped`, flips the fleet, clears `device_id`, keeps `moved_to_device_at`, and writes the audit row with `released` and `closed_deliveries`. A second simultaneous move is refused. EXECUTE revoked from `public`, `anon` and `authenticated` by name; `proacl` read back as `postgres` and `service_role` only. Recorded as version `20260923105330`. Proven live on practice rows, the release identical to the ban's column for column, all deleted afterwards |
| 09-23 | `handle_joins_match_platform` | PF-08 follow-up: the four objects that matched view rows to `accounts` on the handle alone now match handle AND platform — `v_account_view_health` (the `perf` join), `v_account_health_v3` (the `recent` CTE, whose stream now carries a platform), and the three accounts joins in both `analytics_rollup` and `analytics_rollup_fleet`. Before it, a Facebook row sharing its Instagram twin's handle would have borrowed that account's posts and medians and doubled its character's analytics totals. Replaced in place, because n8n reads v3 (View-Collapse Detector, and the Smart Scheduler through `v_scheduler_account_config_all`) and `run_account_health_check` reads `v_account_view_health`. Parity proven before applying (new bodies in a throwaway schema, same statement): both views 64 rows, EXCEPT ALL both ways 0; `analytics_rollup` 12 of 12 range/platform results identical; `analytics_rollup_fleet` 33 of 36 — the 3 Physical + all-time calls never finish on the live function either (see the note under "A fleet function and its original are a pair"). Proven on practice IG + FB accounts sharing one handle inside a rolled-back transaction: before, FB read 2 posts / median 2000 and the character 4 posts / 8000 views; after, FB 0 and the character 2 / 4000. ACLs read back unchanged. Recorded as version `20260923131045`; MD5 of the recorded statement matches the file |
| 09-23 | `calendar_hand_delivery` | P11 (Garreth approved the design the same day): `calendar_day_detail_fleet` gains `hand` (the hand-out's To-do status: queued, posted, postedNoLink, failed, skipped; NULL for Geelark rows) and `post_url`; `calendar_month_days_fleet` gains `link_needed`. Dropped and created again in one migration because the return type changed; every column they returned before is untouched. Proven: fingerprints of both functions over 2026-09-02..24 for all three fleets identical before and after (1,012 day rows, 26 month rows); on real accounts moved to Physical inside a rolled-back transaction, five hand-outs read back as posted + link, postedNoLink (a blank link counts as none), failed with its note, queued and skipped, and the month counted 1 link needed; nothing left behind (0 deliveries, 0 Physical accounts). ACLs read back as `postgres` and `service_role` only. Recorded as version `20260923140901`; MD5 of the recorded statements matches the file. The Geelark-only originals are untouched |
...
because a handle is unique only within a platform. Since 09-23 the same holds
for every join from a view row to `accounts`, not only the gate. One trap when
proving a pair: `analytics_rollup_fleet(null, …, 'physical')` never returns
while the Physical fleet has no posts — `slots` starts its date series at
`'-infinity'` — so an all-time Physical comparison times out on the live
function as well. It is a known fault, not a sign the new body is wrong.

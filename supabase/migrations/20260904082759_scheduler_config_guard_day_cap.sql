-- Snapshot the current view so the change can be proven non-breaking.
create table if not exists _cfg_before_20260904 as
select * from v_scheduler_account_config;

-- Additive only: appends guard_day_cap. No existing column's value changes.
-- guard_day_cap = the ceiling the AGE RAMP and HEALTH THROTTLE impose, ignoring
-- whatever override is set. The dashboard needs it to know which direction of a
-- daily-cap change is even possible: eff_day_cap already folds the override in,
-- so it cannot answer "how high could this go?".  NULL = no guard applies.
create or replace view public.v_scheduler_account_config as
 WITH fleet AS (
         ...
         SELECT r.geelark_profile, r."character", r.platform, r.account_created_on, r.age_days,
            ...
            r.ramp_day_cap, r.ramp_glp_cap,
            LEAST(COALESCE(r.ramp_only_cap, 2147483647), COALESCE(p.max_posts_per_day, 2147483647)) AS guard_cap,
            ...
            LEAST(r.ramp_day_cap, COALESCE(p.max_posts_per_day, r.ramp_day_cap)) AS eff_day_cap,
            LEAST(r.ramp_glp_cap, COALESCE(p.max_glp_per_day, r.ramp_glp_cap)) AS eff_glp_cap
           FROM ramped r
 ...
 SELECT geelark_profile, "character", platform, account_created_on, age_days, health,
    ...
    NULLIF(guard_cap, 2147483647) AS guard_day_cap
   FROM resolved;